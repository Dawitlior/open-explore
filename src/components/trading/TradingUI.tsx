...
import { T } from '@/lib/trading-theme';
...
export const GlassCard = ({ children, style, glow, onClick }: GlassCardProps) => (
...
export const MetricCard = ({ label, value, suffix, color, small }: MetricCardProps) => (
  <GlassCard glow={color === T.accent.cyan ? T.accent.cyanGlow : color === T.accent.red ? T.accent.redGlow : color === T.accent.green ? T.accent.greenGlow : null} style={{ minWidth: small ? 130 : 150, flex: 1 }}>
...
export const ScoreGauge = ({ score, label, color }: ScoreGaugeProps) => {
  const c = 2 * Math.PI * 40;
  const off = c - (score / 100) * c;
  return (
    <GlassCard glow={color === T.accent.cyan ? T.accent.cyanGlow : null} style={{ textAlign: 'center', minWidth: 170, flex: 1 }}>
