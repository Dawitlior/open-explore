...
function BacktestApp({ onReturn }: { onReturn: () => void }) {
  ...
  return <div className="ox" style={{
    ...
    {stratPrompt&&<div style={{position:"fixed",inset:0,zIndex:8600,background:"rgba(2,8,20,.65)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,animation:"btFade .18s ease-out"}} onClick={()=>setStratPrompt(null)}>
      <div className="pop" onClick={(e:any)=>e.stopPropagation()} dir={L==='he'?'rtl':'ltr'} style={{background:BG3,borderRadius:16,padding:24,border:`1px solid ${BRD}`,width:"min(440px,96vw)",boxShadow:"0 30px 80px rgba(0,0,0,.55)",animation:"btPop .28s cubic-bezier(.2,.9,.25,1.15)"}}>
