...
function BacktestApp({ onReturn }: { onReturn: () => void }) {
  ...
  return <div className="ox" style={{
    ...
    {tab==="trades"&&<div style={{padding:"clamp(12px,2vw,20px)",maxWidth:1100,margin:"0 auto"}}>
      ...
      {showFilters&&<div className="fi" style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap",background:BG3,borderRadius:8,padding:"8px 10px",border:`1px solid ${BRD}`}}><input value={search} onChange={(e:any)=>setSearch(e.target.value)} placeholder={S.search} dir={isRTL?"rtl":"ltr"} style={{...inp,flex:1,minWidth:80}}/><select value={filterDir} onChange={(e:any)=>setFilterDir(e.target.value)} style={{...inp,width:"auto"}}><option value="all">{S.all}</option><option value="Long">{S.long}</option><option value="Short">{S.short}</option></select>{uCoins.length>1&&<select value={filterCoin} onChange={(e:any)=>setFilterCoin(e.target.value)} style={{...inp,width:"auto"}}><option value="all">{S.allCoins}</option>{uCoins.map((c:string)=><option key={c} value={c}>{c}</option>)}</select>}{uStrategies.length>=1&&<select value={filterStrategy} onChange={(e:any)=>setFilterStrategy(e.target.value)} style={{...inp,width:"auto",borderColor:filterStrategy!=="all"?BL:BRD,color:filterStrategy!=="all"?BL:T1}}><option value="all">{S.allStrategies}</option>{uStrategies.map((s:string)=><option key={s} value={s}>{s}</option>)}</select>}<select value={sortBy} onChange={(e:any)=>setSortBy(e.target.value)} style={{...inp,width:"auto"}}><option value="date_d">{S.newToOld}</option><option value="date_a">{S.oldToNew}</option><option value="r_d">R↓</option><option value="r_a">R↑</option></select></div>}
