...
const SECTIONS_HE: Section[] = [
  ...
  { icon: '📅', title: 'Calendar Hub — מרכז ה-P&L', items: ['הקלנדר הוא ה-Hub המרכזי של המערכת','לוח שנה אינטראקטיבי עם צבעי רווח/הפסד','מספר עסקאות יומי + סיכום שבועי','שכבת אירועים מאקרו T1 (USD/CNY בלבד)','הדגשת ימי חריגה מסיכון','הערות יום (Day Notes) — שמירה אוטומטית פר-משתמש ופר-תיק עם מצב צפייה/עריכה','יצירת תובנות AI מתוך הקלנדר','תג תזכורת אדום לימי שישי ו-1 לחודש','באנר כיול-מחדש לתודעת הסוחר'] },
  ...
  { icon: '🔊', title: 'ORCA Investment Sound Engine', items: ['סינתזה ב-Web Audio API באיכות גבוהה','מאסטר וולום + הפעלה/כיבוי גלובלי','צלילים נפרדים לפעולות (Add, Lock, Alert, Win, Loss)','Gating מרכזי דרך soundsAllowed()'] },
...
const SECTIONS_EN: Section[] = [
  { icon: '🎨', title: 'Interface & UX', items: ['Premium dark UI with glass, lighting and Liquid Sweep effects','Full Hebrew (RTL) and English with instant switching','Sidebar nav + dynamic mobile popover menu','4 themes: Midnight, Indigo Noir, Platinum White and Graphite Formal (gray · green · red)','Cinematic transitions with full dimensional isolation','Glass cards and subtle micro-interactions','Privacy Mask for instant masking of financial data','Enterprise-grade mobile optimization (safe-area, 100dvh)','44px touch targets, adaptive density, no iOS zoom','Built-in Cookie Consent banner and Legal Gate'] },
  ...
  { icon: '🔊', title: 'ORCA Investment Sound Engine', items: ['High-quality Web Audio API synthesis','Master volume + global on/off','Distinct sounds for actions (Add, Lock, Alert, Win, Loss)','Central gating via soundsAllowed()'] },
  ...
  { icon: '🔐', title: 'Auth, Security & PWA', items: ['Google OAuth only (passwordless)','RequireAuth guards every route','Per-user RLS on every table','PWA — install as a Desktop/Mobile app','Service Worker with cache strategy'] },
