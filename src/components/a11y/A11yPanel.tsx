    ...
    <Dialog.Root open={open} onOpenChange={setOpen}>
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 20px', borderBottom: `1px solid ${SURF.border}` }}>
            <Dialog.Title id={titleId} style={{ fontSize: 17, fontWeight: 700, color: SURF.text1, display: 'flex', alignItems: 'center', gap: 10, margin: 0 }}>
              <Accessibility size={22} color={JC.gold} aria-hidden="true" />
            ...
            <Dialog.Close asChild>
              <button
                ...
                style={{ width: 36, height: 36, borderRadius: 10, background: SURF.card2, border: `1px solid ${SURF.border}`, color: SURF.text2, cursor: 'pointer', display: 'grid', placeItems: 'center' }}
              >
          ...
          <Dialog.Description asChild>
            <p style={{ fontSize: 12, color: SURF.text2, padding: '12px 20px 0', margin: 0, lineHeight: 1.6 }}>
              {t(
          ...
          <div style={{ overflowY: 'auto', padding: '14px 20px 18px', flex: 1 }}>
            ...
            <Section label={t('גודל טקסט', 'Text size')}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ flex: 1, textAlign: 'center', fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: 18, color: SURF.text1 }} aria-live="polite">
                  {scalePct}%
                  <small style={{ display: 'block', fontSize: 10, color: SURF.text2, fontWeight: 500, letterSpacing: '.06em' }}>
                    {t('100% — 200%', '100% — 200%')}
                  </small>
                </div>
            ...
            <button
              type="button"
              onClick={reset}
              style={{ width: '100%', marginTop: 8, padding: '12px 14px', borderRadius: 12, background: 'transparent', border: `1px solid ${SURF.border}`, color: SURF.text2, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontWeight: 600 }}
            >
              <RotateCcw size={16} aria-hidden="true" />
              {t('אפס הגדרות נגישות', 'Reset accessibility settings')}
            </button>

            <footer style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${SURF.border}`, textAlign: 'center' }}>
              <RouterLink
                to="/accessibility"
                onClick={() => setOpen(false)}
                className="a11y-statement-link"
                style={{
                  fontSize: 13, fontWeight: 600, color: JC.gold,
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <section style={{ marginBottom: 18 }}>
      <h3 style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.14em', color: JC.gold, textTransform: 'uppercase', margin: '4px 0 10px' }}>{label}</h3>
      {children}
    </section>
  );
}
...
function SizeBtn({ children, onClick, disabled, ariaLabel }: { children: React.ReactNode; onClick: () => void; disabled?: boolean; ariaLabel: string }) {
  return (
    <button type="button" onClick={onClick} disabled={disabled} aria-label={ariaLabel} className="a11y-size-btn" style={{
      flex: 1, height: 48, borderRadius: 12, background: SURF.card2, border: `1px solid ${SURF.border}`,
      color: disabled ? SURF.text3 : SURF.text1, cursor: disabled ? 'not-allowed' : 'pointer',
...
function segBtn(pressed: boolean): React.CSSProperties {
  return {
    ...
    background: pressed ? `linear-gradient(135deg, ${JC.gold}, ${JC.gold})` : SURF.card2,
    border: pressed ? '1px solid transparent' : `1px solid ${SURF.border}`,
    color: pressed ? JC.onAccent : SURF.text2,
  ...
  return (
    <button
      ...
      style={{
        ...
        width: '100%', background: SURF.card, border: `1px solid ${SURF.border}`, borderRadius: 13,
        padding: '12px 14px', marginBottom: 8, cursor: 'pointer', color: SURF.text1,
        fontFamily: "'Heebo', system-ui, sans-serif", textAlign: 'start',
      }}
    >
      <span style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
        <span style={{ width: 32, height: 32, borderRadius: 9, background: SURF.card2, display: 'grid', placeItems: 'center', color: JC.gold, flex: 'none' }}>{icon}</span>
        <span style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <b style={{ fontSize: 14, fontWeight: 600 }}>{title}</b>
          <span style={{ fontSize: 11.5, color: SURF.text3, marginTop: 2 }}>{hint}</span>
        </span>
      </span>
      <span aria-hidden="true" style={{
        flex: 'none', width: 44, height: 25, borderRadius: 999,
        background: pressed ? `linear-gradient(135deg, ${JC.gold}, ${JC.gold})` : SURF.borderStrong,
